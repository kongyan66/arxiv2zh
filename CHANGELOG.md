...
- Complete and import translations when the hjfy result omits the paper title,
  using the title already retrieved from arXiv metadata.
...
- Fall back to the public arXiv Atom API when `hjfy.top` cannot return
  `arxivInfo`, and show concise network errors instead of raw server output.
