...
<p align="center">
  在 Zotero 中优先翻译 arXiv 源码，缺少源码时改用 PDF 上传，并自动归档中文译文。
</p>
...
arxiv2zh 是一款面向 Zotero 7-10 的轻量级论文翻译插件。无需配置 API Key，
也无需部署本地翻译环境；安装插件并登录 [hjfy.top](https://hjfy.top/)
后，即可在 Zotero 中提交翻译任务。插件优先使用 arXiv 提供的 TeX 源码翻译并
重新编译，以获得更好的公式、图表和版式体验。找不到 arXiv ID 或源码时，
插件会改用条目已附的 PDF 上传翻译；这一模式通常比源码翻译更慢。
译文会自动归档到对应的 Zotero 条目。
...
- 优先使用 arXiv 源码；无 arXiv ID 或无可处理源码时，自动上传条目已附的 PDF。
...
- 论文有可处理的 arXiv 源码，或普通 Zotero 条目已附有本地 PDF（不超过 50MB）。
...
选中普通条目或其原始 PDF 附件，右键选择
“arxiv2zh → 翻译为中文”。可以多选条目批量提交。

插件先从条目的 URL、DOI、Extra 和附件来源中查找 arXiv ID。找到后优先使用
arXiv 源码翻译，以获得更好的排版体验。没有识别到 ID，或服务明确报告论文
没有 LaTeX 源码时，插件会自动将该条目已附的原始 PDF 上传到 hjfy.top 的
本地文档模式。PDF 模式需要处理页面内容，速度通常会下降，译文版式也取决于
原始 PDF。没有可用附件时，插件会提示补充 PDF 或输入 arXiv ID。PDF 上传需
登录，文件不得超过 50MB；相同文件可能共享翻译结果，私密文档请勿上传。
...
`{arxiv-id}_zh_CN.pdf`；PDF 上传模式使用 `{原文件名（不含 .pdf）}_zh_CN.pdf`。
...
| 原始 PDF（仅回退时）                               | 上传到配置的 hjfy.top 服务进行翻译；服务可能共享相同文件的翻译结果 |
...
- PDF 上传回退需要普通 Zotero 条目下的本地原始 PDF 附件，不支持独立附件。
...
- **提示没有 LaTeX 源码**：给条目添加本地原始 PDF 附件后重试，插件会改用上传模式。
